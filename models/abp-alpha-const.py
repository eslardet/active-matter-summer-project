"""
Two population ABP model with different (constant) persistences
Euler method for solving SDEs
Uses a combined Verlet/ linked list neighbour list method for efficiency of the force function
"""
...
def two_populations_alpha(N_particles, phi, Pe, alpha_A, alpha_B, rdiff, t, sample, delta_t, file):
    ...
    # alpha_B: alpha value of population B particles
    ...
    a_sqrt = np.sqrt(alpha_A)
    b_sqrt = np.sqrt(alpha_B)
    C_rot = np.concatenate((np.full(N_A, a_sqrt), np.full(N_B, b_sqrt))) * C_trans
